...
int GS(int n, double *b, double **A, double *X_0, double conv, double *X_sol, int maxIter){
...
int main(){
	...
	//printf("norm %lf \n", a)
	int numIterations = GS(n, b, A, X_0, conv, X_sol, maxIter);
	
	// displayVector(n, X_sol) ;

	FILE *fileSol = fopen("GSSolution.txt", "w");
	...
	FILE *fileSolInfo = fopen("GSSolutionInfo.txt", "w");
