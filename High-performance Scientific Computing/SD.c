...
int steepest_descent(int n, double **A, double *b, double *X_0, double *X_sol, double* residual_arr, double conv, int maxIter){
...
int main(){
    ...
    numIterations = steepest_descent(n, A, b, X_0, X_sol, residual_arr, conv, maxIter);
	...
	FILE *fileSol = fopen("../solution_data/SD/SDSolution.txt", "w");
    if(fileSol == NULL){
	...
	FILE *fileSolInfo = fopen("../solution_data/SD/SDInfo.txt", "w");
	fprintf(fileSolInfo, "%d\n", numIterations) ;
	fprintf(fileSolInfo, "%.12f", cpu_time_used);
	fclose(fileSolInfo);
	
    FILE *fileSolRes = fopen("../solution_data/SD/SDResidual.txt", "w");
    if(fileSolRes == NULL){
