classdef inputData
    ...
    methods
        function obj = inputData(NNODE, ID, X, Y, PRES_DISP, NODES, E, nu, thickness, BODY_FORCE, SURFACE_FORCE, SURFACE_FORCE_INFO, CONC_FORCE, APPROXIMATION_ORDER, NUM_INTEG_POINTS)
            obj.NNODE = NNODE;
            ID = createID(ID);
            ...
            function [LM] = createLM(ID, NODES, APPROXIMATION_ORDER)
                row = size(NODES, 1) ;
                LM = zeros(row, 2*4*APPROXIMATION_ORDER);
            
             
                for i=1:row
                    for j=1:4*APPROXIMATION_ORDER
                        
                        LM(i, 2*j-1) = ID(1, NODES(i, j));
                        LM(i, 2*j)   = ID(2, NODES(i, j));
