...
NNODE = 21;
NELEM = 4;
APPROXIMATION_ORDER = 2 ; % can be 1 or 2 
...
BODY_FORCE = {@(X, Y) 0 , @(X, Y) 0;
              @(X, Y) 0 , @(X, Y) 0;
              @(X, Y) 0 , @(X, Y) 0;
              @(X, Y) 0 , @(X, Y) 0;}; % NELEM X 2
SURFACE_FORCE = {@(t) 1, @(t) 0}; 
SURFACE_FORCE_INFO = [3 4 4 4] ;%  element_number, edge_number, element_number, edge_number
